...
// 编译运行（在仓库根目录）：
//   g++ -std=c++23 -Iinclude test/test_ib_pinyin.cpp src/pinyin.cpp -Llib
//   -lib_pinyin_c -o test/test_ib_pinyin LD_LIBRARY_PATH=lib
//   test/test_ib_pinyin
#include <cassert>
#include <cstdio>
#include <string_view>
...
static void 测试查找匹配() {
  auto 结果 =
      查找匹配("sousuo", "拼音搜索Everything", 注音方式::全拼 | 注音方式::简拼);
  assert(结果.has_value());
